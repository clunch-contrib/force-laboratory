...
export default function (electrons) {
    ...
    // 求解出坐标最值
    let minX = electrons[0][0], minY = electrons[0][1], maxX = electrons[0][0], maxY = electrons[0][1];
    ...
    (function calc_Q_Tree(nodes, id, ix, ax, iy, ay) {
        ...
        let ltNodes = [], rtNodes = [], lbNodes = [], rbNodes = [];
    ...
    // 求解库仑力
    let treeNode, eleNode, law = [], d2, r2,
        /**
        ...
        doLaw = function (q1, x1, y1, x2, y2) {
            if (x1 == x2 && y1 == y2)
                // 重叠的点忽略
                return [0, 0];
            let f = q1 / ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
    ...
    let calc_Coulomb_Law = function (treeName, i) {
